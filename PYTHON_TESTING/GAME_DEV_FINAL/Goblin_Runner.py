...
class Goblin_Runner:
    ...
    def __init__(self):
        ...
        pg.display.set_caption("Goblin Runner")
        ...
        while running:
                ...
                # If a key is pressed down and pressed 'q', the game will exit
                    ...
                    # If Spacebar is pressed, the following decision will be made.
                        ...
                        if self.game_dialogue.is_dialogue_active():
                            if not self.game_dialogue.advance_dialogue(): # Advance dialogue, if it ends.
            ...
            # If in level dialogue, draw level background and dialogue box and text.
             ...
             # If game is over, draw game over screen.
                ...
                self.game_level._draw_game_over_screen(self.screen)
             ...
             # If in credits screen, allow menu to handle events and draw credits.
                ...
                self.menu.handle_events(events)
                self.menu._draw_credits_screen()

            # If in active gameplay, 
                ...
                self.character.update(keys) # Update character.
                
                if self.current_level <= self.MAX_LEVEL: # If within valid gameplay levels.
                    self.enemy_system.update(self.current_level, self.character.rect) # Update enemy.

                    shrunk_char_rect = self.character.rect.inflate(-self.collision_offset * 2, -self.collision_offset * 2) # Create smaller collision rect for character.
                    enemy_rects_for_level = self.enemy_system.get_current_enemy_rects(self.current_level) # Get enemy collision rectangles.
                    for enemy_rect in enemy_rects_for_level: # Check each enemy for collision.
                        if shrunk_char_rect.colliderect(enemy_rect): # If character collides with enemy.
                            self.game_state = self.STATE_GAME_OVER # Set game to game over.
                            self.game_active = False # Deactivate gameplay.
                            break # Stop checking for more collisions.
                        
                if self.current_level <= self.MAX_LEVEL and self.level_transition_cooldown <= 0: # If not on cooldown and within levels.
                    if self.character.rect.right >= self.screen_width: # If character moves off right side.
                        if self.current_level < self.MAX_LEVEL: # If not the last level.
                            self.current_level += 1 # Advance to next level.
                            self.game_dialogue.set_level_dialogue(self.current_level) # Set new dialogue.
                            self.game_dialogue.start_dialogue() # Start new dialogue.
                            self.game_state = self.STATE_LEVEL_DIALOGUE # Go to dialogue state.
                            self.game_active = False # Deactivate gameplay.
                            self.level_transition_cooldown = self.game_dialogue.COOLDOWN_FRAMES # Reset cooldown.
                            self.character.rect.x = 5 # Reset character X position.
                            self.enemy_system.reset_for_level(self.current_level) # Reset enemy for new level.
                        else: # If it's the last level.
                            self.game_state = self.STATE_GAME_COMPLETED # Set game to completed.
                            self.game_active = False # Deactivate gameplay.
                            self.character.rect.right = self.screen_width # Keep character on screen.
                            self.enemy_system.reset_all_enemies() # Reset all enemies.

                     # If character moves off left side,
                    elif self.character.rect.left <= 0:
                        if self.current_level > 1:
                            # Go to previous level.
                            self.current_level -= 1

                            # Set previous dialogue and start previous dialogue.
                            ...
                            # Reset character position and cooldown
                            self.level_transition_cooldown = self.game_dialogue.COOLDOWN_FRAMES
                            self.character.rect.x = self.screen_width - self.character.rect.width - 5

                            # Reset enemy for previous level.
                            self.enemy_system.reset_for_level(self.current_level)
                        
                        else: # If it's the first level.
                            self.character.rect.left = 1 # Keep character on screen.
                ...
                self.game_level.draw_level_text(self.screen, self.current_level) # Draw level text.
